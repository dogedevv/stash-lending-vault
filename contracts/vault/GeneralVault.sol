...
abstract contract GeneralVault is VersionedInitializable {
  ...
  modifier onlyYieldProcessor() {
    require(_addressesProvider.getAddress("YIELD_PROCESSOR") == msg.sender, Errors.CALLER_NOT_YIELD_PROCESSOR);
    _;
  }
  ...
  function depositCollateral(address _asset, uint256 _amount) external payable virtual {
    ...
    ILendingPool(_addressesProvider.getLendingPool()).deposit(_stAsset, _stAssetAmount, msg.sender, 0);
  ...
  function withdrawCollateral(
    ...
    console.log("_stAssetAmount", _stAssetAmount);

    // withdraw from lendingPool, it will convert user's aToken to stAsset
    uint256 _amountToWithdraw = ILendingPool(_addressesProvider.getLendingPool()).withdrawFrom(_stAsset, _stAssetAmount, msg.sender, address(this));
    ...
    require(withdrawAmount >= _amount.percentMul(PercentageMath.PERCENTAGE_FACTOR - _slippage), Errors.VT_WITHDRAW_AMOUNT_MISMATCH);
  ...
  function setHarvestOnDeposit(bool flag) external onlyAdmin {
   ...
   */
  function _getYieldAmount(address _stAsset) internal view virtual returns (uint256) {
  ...
  function _depositYield(address _asset, uint256 _amount) internal virtual {
   ...
   */
  function _depositToYieldPool(address _asset, uint256 _amount) internal virtual returns (address, uint256);
   ...
   */
  function _getWithdrawalAmount(address _asset, uint256 _amount) internal view virtual returns (address, uint256);
}
